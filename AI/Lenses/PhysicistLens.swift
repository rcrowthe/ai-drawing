...
class PhysicistLens: Lens {
    ...
    func analyze(
        ...
        let energyLevel = calculateEnergyLevel(userStroke)
        let oscillation = detectOscillation(recentStrokes)
        let stabilityScore = calculateStability(recentStrokes)
        ...
        // High energy, unstable - introduce contrast/surprise
        if energyLevel > energyThreshold && stabilityScore < stabilityThreshold {
            suggestions.append((.contrast, 0.6))
            suggestions.append((.surprise, 0.4))
        }

        // Low energy, stable - add gentle texture
        if energyLevel < energyThreshold / 2 && stabilityScore > 0.7 {
        ...
        return LensAnalysis(
            ...
            urgency: energyLevel > energyThreshold ? 0.9 : urgency * weight
        )
    }
